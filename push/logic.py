...
def startPush(
    role: discord.Role,
    membersChannel: discord.TextChannel,
    chatChannel: discord.TextChannel,
    crewName: str,
):
    constants.configCollection.update_one(
        {"key": "crews"}, {"$push": {"value": crewName}}
    )
    constants.configCollection.update_one(
        {"key": "crew_region"}, {"$push": {f"value.PUSH": crewName}}
    )
    constants.vacanciesCollection.update_one(
        {}, {"$set": {crewName: {"current": 0, "next": 0}}}
    )
...
def endPush(crewName):
    constants.configCollection.update_one(
        {"key": "crews"}, {"$pull": {"value": crewName}}
    )
    constants.configCollection.update_one(
        {"key": "crew_region"}, {"$pull": {f"value.PUSH": crewName}}
    )
