...
class EndPushView(discord.ui.View):
    def __init__(self, ctx: discord.ApplicationContext):
        ...
        self.crew_name = None
        utils.resetSelect(
            self, {"crew": utils.getPushCrewNames(constants.configCollection)}
        )

    @discord.ui.string_select(placeholder="Push crew", custom_id="crew")
    async def callback(
...
class StartPushView(discord.ui.View):
    ...
    async def maybeSendModal(self, interaction: discord.Interaction):
        if (
            ...
            and self.members_channel is not None
            and self.chat_channel is not None
        ):
            await interaction.response.send_modal(
                modals.StartPushModal(
                    self.ctx, self.role, self.members_channel, self.chat_channel
                )
            )
        else:
    ...
    async def membersChannelSelectCallback(
        self, select: discord.ui.Select, interaction: discord.Interaction
    ):
    ...
    async def chatChannelSelectCallback(
        self, select: discord.ui.Select, interaction: discord.Interaction
    ):
        if isinstance(select.values[0], discord.TextChannel):
